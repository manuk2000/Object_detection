...
def get_position_to_zoom(video_path, left_zoom, right_zoom, size, times_left_right_zoom):
    ...
    while True:
        ...
        zoomed_frame = zoomed_frame[scrole.get("top"):scrole.get("bottom"), scrole.get("left"): scrole.get("right")]
        ...
        if ((time_index < len(time) - 1) and (video_time >= time[time_index])):
            # Check if the video_time is greater or equal to the next time point
            time_index += 1
            print("\n\n\n")
...
def main(video_path, zoom_position, times_left_right_zoom):
    ...
    get_position_to_zoom(video_path, left_zoom, right_zoom, size, times_left_right_zoom)


# Read the zoom position data from the file
